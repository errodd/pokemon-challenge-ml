# Plantilla de Presentacion PowerPoint

## Instrucciones de uso
- Esta plantilla esta pensada para convertirla en diapositivas 1:1.
- En cada diapositiva reemplaza los bloques [COMPLETAR] con tu informacion.
- No incluyas codigo en la presentacion.
- Cada grafico o tabla debe ir con interpretacion y mensaje de negocio.
- Usa formato numerico consistente de 3 decimales en metricas.
- Duracion sugerida total: 10 a 15 minutos.

---

## Diapositiva 1: Portada
**Titulo:** [Titulo del proyecto]

**Subtitulo:** [Prediccion del ganador de batallas Pokemon]

**Equipo:** [Nombres]

**Fecha:** [DD/MM/AAAA]

**Mensaje clave (1 frase):**
[COMPLETAR]

---

## Diapositiva 2: Agenda
1. Contexto del problema
2. Entendimiento del dataset
3. Insights del EDA
4. Data Preparation
5. Modelado
6. Evaluacion de modelos
7. Seleccion del modelo final
8. Interpretabilidad
9. Limitaciones
10. Proximos pasos
11. Conclusiones

---

## Diapositiva 3: Contexto del problema
**Que problema resolvemos:**
[COMPLETAR]

**Tipo de problema (clasificacion o regresion):**
[COMPLETAR]

**Caso de uso real / relevancia:**
[COMPLETAR]

**Mensaje clave:**
[COMPLETAR]

---

## Diapositiva 4: Entendimiento del dataset
**Origen de datos:**
- [Fuente 1]
- [Fuente 2]

**Que representa cada fila:**
- [Tabla A]
- [Tabla B]

**Tamano del dataset:**
- [N filas tabla A]
- [N filas tabla B]

**Mensaje clave:**
[COMPLETAR]

---

## Diapositiva 5: Variables del dataset
Usa una tabla simple:

| Grupo | Variables | Rol |
|---|---|---|
| Numericas | [COMPLETAR] | [COMPLETAR] |
| Categoricas | [COMPLETAR] | [COMPLETAR] |
| Binarias | [COMPLETAR] | [COMPLETAR] |
| Objetivo | [COMPLETAR] | [COMPLETAR] |
| Excluidas por leakage | [COMPLETAR] | [COMPLETAR] |

**Interpretacion (2-3 lineas):**
[COMPLETAR]

---

## Diapositiva 6: Principales insights del EDA (hallazgos)
**Hallazgo 1:** [COMPLETAR]
- Evidencia: [COMPLETAR]
- Implicacion para modelado: [COMPLETAR]

**Hallazgo 2:** [COMPLETAR]
- Evidencia: [COMPLETAR]
- Implicacion para modelado: [COMPLETAR]

**Hallazgo 3:** [COMPLETAR]
- Evidencia: [COMPLETAR]
- Implicacion para modelado: [COMPLETAR]

---

## Diapositiva 7: EDA y calidad de datos
**Problemas detectados:**
- Missing values: [COMPLETAR]
- Duplicados: [COMPLETAR]
- Outliers / sesgos: [COMPLETAR]
- Riesgos de leakage: [COMPLETAR]

**Por que es importante para modelar:**
[COMPLETAR]

---

## Diapositiva 8: Data Preparation
**Transformaciones principales:**
- [COMPLETAR]
- [COMPLETAR]
- [COMPLETAR]

**Manejo de faltantes:**
[COMPLETAR]

**Encoding de categoricas:**
[COMPLETAR]

**Feature engineering realizado:**
[COMPLETAR]

**Justificacion de decisiones:**
[COMPLETAR]

---

## Diapositiva 9: Modelado
**Modelos evaluados (minimo 3):**
- Baseline: [COMPLETAR]
- Modelo 2: [COMPLETAR]
- Modelo 3: [COMPLETAR]
- (Opcional) Modelo 4: [COMPLETAR]

**Por que se eligieron:**
[COMPLETAR]

---

## Diapositiva 10: Metricas de evaluacion
**Metricas usadas:**
- [Metrica 1]
- [Metrica 2]
- [Metrica 3]
- [Metrica 4]

**Que mide cada metrica (en lenguaje simple):**
- [Metrica 1]: [COMPLETAR]
- [Metrica 2]: [COMPLETAR]
- [Metrica 3]: [COMPLETAR]
- [Metrica 4]: [COMPLETAR]

---

## Diapositiva 11: Comparacion de modelos

Nota metodologica:
- CV-F1 se usa para comparar todos los modelos.
- Accuracy, Balanced Accuracy y ROC-AUC deben reportarse en test solo para el modelo final seleccionado (si no aplica, usar N/D).

| Modelo | CV-F1 | Accuracy | Balanced Accuracy | ROC-AUC | Comentario |
|---|---:|---:|---:|---:|---|
| [Baseline] | [ ] | N/D | N/D | N/D | [COMPLETAR] |
| [Modelo 2] | [ ] | N/D | N/D | N/D | [COMPLETAR] |
| [Modelo 3] | [ ] | N/D | N/D | N/D | [COMPLETAR] |
| [Modelo final] | [ ] | [ ] (test) | [ ] (test) | [ ] (test) | [COMPLETAR] |

**Interpretacion de la comparacion:**
[COMPLETAR]

---

## Diapositiva 12: Seleccion del modelo final
**Modelo seleccionado:** [COMPLETAR]

**Criterios de seleccion (no solo mejor metrica):**
- Rendimiento: [COMPLETAR]
- Robustez / generalizacion: [COMPLETAR]
- Complejidad y costo computacional: [COMPLETAR]
- Alineacion con el problema: [COMPLETAR]

**Mensaje clave:**
[COMPLETAR]

---

## Diapositiva 13: Interpretabilidad
**Variables mas importantes:**
1. [COMPLETAR]
2. [COMPLETAR]
3. [COMPLETAR]
4. [COMPLETAR]

**Como se interpreta el comportamiento del modelo:**
[COMPLETAR]

**Decision practica que habilita:**
[COMPLETAR]

---

## Diapositiva 14: Limitaciones
**Limitaciones del dataset:**
- [COMPLETAR]
- [COMPLETAR]

**Riesgos metodologicos (incluye leakage):**
- [COMPLETAR]
- [COMPLETAR]

**Restricciones del enfoque:**
- [COMPLETAR]
- [COMPLETAR]

---

## Diapositiva 15: Proximos pasos
**Mejoras inmediatas:**
1. [COMPLETAR]
2. [COMPLETAR]
3. [COMPLETAR]

**Que hariamos con mas tiempo o mas datos:**
1. [COMPLETAR]
2. [COMPLETAR]
3. [COMPLETAR]

---

## Diapositiva 16: Conclusion
**Resumen de resultados (3 bullets):**
- [COMPLETAR]
- [COMPLETAR]
- [COMPLETAR]

**Que se puede concluir del modelo:**
[COMPLETAR]

**Que decisiones permite tomar:**
[COMPLETAR]

---

## Diapositiva 17: Cierre y preguntas
**Gracias**

**Contacto:** [COMPLETAR]

**Q&A**

---

## Checklist final antes de exponer
- No hay codigo en ninguna diapositiva.
- Cada tabla o grafico tiene interpretacion.
- Se explica por que cada decision metodologica fue tomada.
- El modelo final esta justificado con contexto, no solo por puntaje.
- Las limitaciones y siguientes pasos estan explicitos.
